...
class CustomStatus
{
    ...
    public static function addOrderStates(int $language_id, $base_name)
    {
        ...
        foreach ($mails_languages as $l) {
            ...
            if ( is_dir(_PS_ROOT_DIR_.'/mails/'.$l.'/' ) ) {
                copy(_PS_MODULE_DIR_.'/bluepayment/mails/'.$l.'/bluemedia_pending.html', _PS_ROOT_DIR_.'/mails/'.$l.'/bluemedia_pending.html');
                copy(_PS_MODULE_DIR_.'/bluepayment/mails/'.$l.'/bluemedia_pending.txt', _PS_ROOT_DIR_.'/mails/'.$l.'/bluemedia_pending.txt');
                copy(_PS_MODULE_DIR_.'/bluepayment/mails/'.$l.'/bluemedia_completed.html', _PS_ROOT_DIR_.'/mails/'.$l.'/bluemedia_completed.html');
                copy(_PS_MODULE_DIR_.'/bluepayment/mails/'.$l.'/bluemedia_completed.txt', _PS_ROOT_DIR_.'/mails/'.$l.'/bluemedia_completed.txt');
                copy(_PS_MODULE_DIR_.'/bluepayment/mails/'.$l.'/bluemedia_payment_error.html', _PS_ROOT_DIR_.'/mails/'.$l.'/bluemedia_payment_error.html');
                copy(_PS_MODULE_DIR_.'/bluepayment/mails/'.$l.'/bluemedia_payment_error.txt', _PS_ROOT_DIR_.'/mails/'.$l.'/bluemedia_payment_error.txt');
            }
    ...
    private static function checkIfStateExists(string $name, $language_id) : bool
    {
        ...
        foreach ($states as $state)
            if (in_array($name, $state)) return true;
...
//        var_dump(OrderState::getOrderStates($language_id)); die();
        foreach ($states as $state) {
