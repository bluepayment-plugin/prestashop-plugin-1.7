...
class Transformer
{
    ...
    public static function modelToArray(ItnIn $model)
    {
        ...
        if ($model->getVerificationStatusReasons()) {
            $result['verificationStatusReasons'] = $model->getVerificationStatusReasons();
        }
    ...
    public static function toModel(SimpleXMLElement $xml)
    {
        ...
        if (isset($transaction->verificationStatusReasons->verificationStatusReason)) {
            $reasons = (array) $transaction->verificationStatusReasons->verificationStatusReason;
            $diff = array_diff($reasons, ItnIn::VERIFICATION_STATUSES);

            if (!empty($diff)) {
                Logger::log(
                    Logger::EMERGENCY,
                    sprintf('Not supported verificationStatusReason="%s"', json_encode($diff)),
                    ['itn' => $xml]
                );
            } else {
                $model->setVerificationStatusReasons($reasons);
            }
        }
